...
public class LudoMultiplayerView extends View {
    ...
    class C03871 implements Comparator<Drawable> {
        C03871() {
        }
    ...
    class GameLoopHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            LudoMultiplayerView.this.invalidate();
            LudoMultiplayerView.this.gameLoop();
        }
    ...
    public LudoMultiplayerView(Context context) {
        super(context);
        ...
        this.f50k = Integer.valueOf(0);
        this.f51l = Integer.valueOf(0);
    ...
    public LudoMultiplayerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.currentBoxPlayer = new Integer[]{Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(0)};
        this.playerOutOfGame = new Integer[]{Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(0)};
        this.playerRankList = new ArrayList();
    ...
    public LudoMultiplayerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (this.playerNow.getPlayerId() == i) {
                if (this.blinkMarker == 0) {
                    color = (Paint) this.colorList.get(i);
                } else {
                    color = (Paint) this.colorListBlink.get(i);
                }
            }
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (j = 0; j < 4; j++) {
                ...
                canvas3.drawCircle(gridBox.getX() * this.gridBoxDpWidth, gridBox.getY() * this.gridBoxDpHeight, this.gridBoxDpWidth / 2.0f, (Paint) this.colorList.get(i));
        ...
        if (this.gameComplete.booleanValue()) {
            ...
            Canvas canvas2 = canvas;
            Canvas canvas3 = canvas2;
            canvas3.drawText("Game Over", ((((gridBox0.getX() * this.gridBoxDpWidth) + (gridBox1.getX() * this.gridBoxDpWidth)) + this.gridBoxDpWidth) * 2.0f) / 5.0f, (((gridBox0.getY() * this.gridBoxDpHeight) + (gridBox1.getY() * this.gridBoxDpHeight)) + this.gridBoxDpHeight) / 2.0f, this.gameOverPaint);
            canvas2 = canvas;
            canvas3 = canvas2;
            canvas3.drawText("Click to restart.", ((((gridBox0.getX() * this.gridBoxDpWidth) + (gridBox1.getX() * this.gridBoxDpWidth)) + this.gridBoxDpWidth) * 2.0f) / 5.0f, (((gridBox0.getY() * this.gridBoxDpHeight) + (gridBox1.getY() * this.gridBoxDpHeight)) + (2.0f * this.gridBoxDpHeight)) / 2.0f, this.gameOverPaint);
        }
    }
    ...
    private void gameLoop() {
        Collections.sort(this.markersInGameList, new C03871());
        this.gameLoopHandler.sleep(this.gameLoopDelay);
    }

    private void play() {
        if (LudoMultiplayerActivity.play.booleanValue()) {
            switch (this.gameState.intValue()) {
                case 0:
                    ...
                    return;
                case 1:
                    if (this.f51l.intValue() < this.maxDiceRolls.intValue()) {
                        this.f51l = Integer.valueOf(this.f51l.intValue() + 1);
